...
func (m *MockExpenseRepository) ReassignCategoryFromMonth(ctx context.Context, fromCategoryID expense.ID, toCategoryID expense.ID, month string) error {
	args := m.Called(ctx, fromCategoryID, toCategoryID, month)
	return args.Error(0)
}
